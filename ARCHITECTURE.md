...
MCP-сервер для Cursor: семантический поиск по документации конфигурации 1С. XML-выгрузка индексируется на backend-сервере; агент в Cursor работает через MCP tools (`conf-doc mcp` → HTTP API).

## Обзор

```
┌──────────────────────────────────────────────────────────────────────────┐
│  Cursor Agent  →  conf-doc mcp (stdio)  →  HTTP API  →  Pipeline       │
└──────────────────────────────────────────────────────────────────────────┘
         │                                        │
         │  conf_doc_search, get_object, …        ▼
         │                              SQLite + FAISS + markdown
         │
  conf-doc CLI (разработка / администрирование)
```

```
┌─────────────────────────────────────────────────────────────────────────┐
│                         Клиенты                                       │
│  MCP (Cursor)  │  conf-doc CLI  │  HTTP API (прямые вызовы)           │
└────────┬───────────────────┬──────────────────────┬───────────────────┘
         │                   │                      │
         └───────────────────┼──────────────────────┘
                             ▼
...
|------|----------------|------------|
| **Точки входа** | `cli.py`, `api/`, `mcp/` | CLI (`index`, `serve`, …), HTTP API (backend), MCP stdio (клиент для Cursor) |
...
| **Слоты выгрузки** | `export_slot.py`, `export_root.py` | `output/exports/{Имя}/`, привязка staging (`.import_source`) |

## Слоты выгрузки (export slots)

Канонический путь XML-выгрузки для конфигурации — `output/exports/{Имя}/`. Импорт:

- **ZIP** — копирование в слот;
- **Путь** (`import-path`) — по умолчанию **привязка** через `.import_source` (без копирования); `mirror: true` — полное зеркалирование.

`Pipeline.index_export()` читает выгрузку через `slot_export_root()`. Индексация разделена на этапы:

| Этап | Веб-UI / API | CLI |
|------|----------------|-----|
| Метаданные + чанки | «Обновить», `POST .../index` + `skip_embeddings: true` | `conf-doc index --skip-embeddings` |
| Эмбеддинги + FAISS | «Переиндексировать», `POST .../embed` | `conf-doc embed` |

Пользовательская документация: [docs/CONFIGURATION_SLOTS.md](docs/CONFIGURATION_SLOTS.md).
...
Команды: `conf-doc index`, `POST /configurations/{name}/index`, `conf-doc embed`, `POST /configurations/{name}/embed`. Устаревший `POST /reindex`. Флаг `--force` / `"force": true` — полная пересборка чанков и эмбеддингов.
...
**Report (отчёты):** при индексации дополнительно читаются `Ext/ObjectModule.bsl` и запросы из основной СКД (`MainDataCompositionSchema` → `Templates/.../Ext/Template.xml`). В markdown появляются секции `## Модуль объекта` и `## Запрос СКД: {набор}` — каждая даёт отдельный searchable чанк с BSL-кодом или полным текстом запроса на языке 1С.
