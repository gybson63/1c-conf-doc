...
**Не путать:** `/search` использует **embedding-модель** (`embeddings.*` в config на сервере), не chat LLM. По умолчанию в Docker — локальная `sentence_transformers`; LLM для `/query` отключён (`llm.provider: none`). Подробнее: [README — Выбор провайдера эмбеддингов](../../README.md#выбор-провайдера-эмбеддингов).
...
- Индексация на сервере: слоты, веб-UI (**Обновить** / **Переиндексировать**) — [docs/CONFIGURATION_SLOTS.md](../../docs/CONFIGURATION_SLOTS.md); API `POST /configurations/{name}/index`, `.../embed`.
